...
# Docs (CI runs this too: regenerate API stubs, then strict build)
...
- `docs`: mkdocstrings-python, mkdocs-material[imaging], mkdocs-literate-nav, griffelib, mkdocs-git-revision-date-localized-plugin, zensical
...
- Zensical theme (`variant: modern`), nav tabs, section hubs with card grids
- API reference via `mkdocstrings` `:::` autodoc stubs generated by `scripts/gen_api_docs.py` (Griffe); nav resolved by `mkdocs-literate-nav` (`navigation.indexes` collapses each API package section into its index page)
- `git-revision-date-localized` (creation dates)

### Commands

```bash
uv run zensical build --strict     # Verify: must exit 0 with zero build warnings
uv run zensical serve              # Live-reload preview
