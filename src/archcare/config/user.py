...
Provides the single source of truth for determining whether the application
is running interactively or under a systemd timer, and handles conditional
file ownership changes when running as root.
...
class UserContext:
    ...
    Resolves the `ARCHCARE_USER` environment variable once per invocation.
    This variable is set by the systemd service unit for scheduled runs;
    its absence indicates the command is running interactively.

    Attributes:
        archcare_user (str | None): The target username from the
            `ARCHCARE_USER` environment variable, or `None` if unset
            (interactive run).
    ...
    def chown_if_root(self, *paths: Path) -> None:
        ...
        See Also:
            - [is_root][archcare.utils.is_root]: Root privilege check
