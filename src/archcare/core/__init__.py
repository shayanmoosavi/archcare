...
Key Concepts:
    ...
    - **Typed results**: [`TaskResult`][] is generic over per-task detail dataclasses
      ([`FailedServicesDetails`][], [`HealthCheckDetails`][], etc.), so
...
Modules:
    ...
    executor: [`TaskExecutor`][archcare.core.executor.TaskExecutor] orchestrating instantiation,
        guards, and state updates.
    exceptions: Core-layer exception hierarchy rooted at `ArchcareCoreError`.
    formatter: [`TaskDetailFormatter`][archcare.core.formatter.TaskDetailFormatter] port for
        rendering task details.
    interaction: [`TaskInteraction`][archcare.core.interaction.TaskInteraction] port for
        confirm/notify during runs.
    ...
    progress: [`TaskProgress`][archcare.core.progress.TaskProgress] port plus no-op and Rich
        implementations.
